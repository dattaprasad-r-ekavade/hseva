...
class ShiftSchemaInstaller
{
    public function install(PDO $db): void
    {
        $db->exec("CREATE TABLE IF NOT EXISTS shift_master (
    ...
    UNIQUE(shift_code)
        ...
        $db->exec("CREATE TABLE IF NOT EXISTS employee_shift_assignments (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    UNIQUE(emp_id)
        ...
        $db->exec("CREATE TABLE IF NOT EXISTS shift_rosters (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    emp_id TEXT NOT NULL,
    ...
    UNIQUE(emp_id, roster_date)
        ...
        $db->exec("CREATE TABLE IF NOT EXISTS shift_roster_weeks (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    UNIQUE(week_start_date, week_end_date)
        ...
        $count = (int) ($db->query('SELECT COUNT(*) AS c FROM shift_master')->fetch()['c'] ?? 0);
        if ($count === 0) {
